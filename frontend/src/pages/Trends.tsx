    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <select className="input w-56" value={vertical} onChange={(e) => setVertical(e.target.value)}>
      ...
      <div className="flex gap-1 border-b border-slate-200">
        {(['hashtags', 'posts', 'tiktok'] as const).map((t) => (
          <button key={t} onClick={() => setTab(t)}
            className={`px-4 py-2 text-sm border-b-2 ${tab === t ? 'border-brand-600 text-brand-700 font-medium' : 'border-transparent text-slate-500'}`}>
        ...
        <div className="card p-5">
          {hashtags.isLoading ? 'Cargando…' : (
            <ul className="grid grid-cols-3 gap-2 text-sm">
              ...
              {hashtags.data?.length === 0 && <li className="col-span-3 text-slate-500">Sin datos aún. Agregá competidores en /competitors para alimentar esto.</li>}
            </ul>
          )}
        </div>
      )}
