    ...
    private static async Task SeedPostingProfilesAsync(ApplicationDbContext db, CancellationToken ct)
    {
        ...
        var profiles = new List<PostingProfile>
        {
            new()
            {
                ...
                ContentPillars = new() { "Cobros automáticos sin perseguir morosos", "Semáforo de acceso en recepción (QR)", "Clases llenas, sin ausencias", "Dashboard único, adiós Excel", "Rutinas personalizadas", "Setup en 1 día incluido", "Sin permanencia, 7 días gratis", "Integrado con MercadoPago + WhatsApp" },
