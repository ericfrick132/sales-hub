    ...
    private static async Task SeedPostingProfilesAsync(ApplicationDbContext db, CancellationToken ct)
    {
        ...
        var profiles = new List<PostingProfile>
        {
            new()
            {
                ...
                ContentPillars = new() { "Cobro de cuotas sin perseguir a nadie a mano (vencimientos claros + semáforo)", "Control de ingreso de alumnos (QR / huella / facial)", "Venta de bebidas y productos desde el sistema", "Rentabilidad clara: cuánto entra, cuánto queda a fin de mes", "Dashboard único, adiós Excel", "Setup en 1 día incluido", "Sin permanencia, 7 días gratis", "Integrado con MercadoPago + WhatsApp" },
                BrandGuidelines = "Idioma es-AR (voseo). Ejemplos de copy: 'Dejá de perseguir morosos. GymHero cobra por vos.' / 'Todo lo que tu gym necesita. Nada que no.' / 'Tu gym merece dejar de perder plata.' Mostrar métricas reales, no fluff. HECHOS DEL PRODUCTO (prohibido inventar mecanismos): los pagos NO 'rebotan' ni existe detección de pagos fallidos — el dolor real del dueño es PERSEGUIR el pago de las cuotas A MANO y controlar el INGRESO de los alumnos. GymHero resuelve: estado de cada cuota al día (semáforo verde/rojo en recepción), control de acceso de alumnos, venta de bebidas/productos desde el sistema, y ver la RENTABILIDAD real del gym. Nunca describir débitos automáticos que fallan ni features que no existen.",
            ...
            new()
            {
                ...
                ContentPillars = new() { "Reservas automáticas 24/7", "Pagos y señas online", "Analytics de ocupación y revenue", "Membresías con auto-cobro", "Recordatorios que evitan ausencias", "Link público branded del club", "Gestión multi-cancha", "Ranking/ELO para torneos" },
