    ...
    private static async Task SeedPostingProfilesAsync(ApplicationDbContext db, CancellationToken ct)
    {
        ...
        var profiles = new List<PostingProfile>
        {
            new()
            {
                ...
                ContentPillars = new() { "Cobro de cuotas sin perseguir: débito automático MercadoPago + recordatorios por WhatsApp", "Semáforo al día/debe en recepción + acceso por QR", "Venta de bebidas y productos: POS con stock y cierre de caja", "Ingresos, gastos, sueldos y comisiones en un solo lugar (adiós Excel)", "Reservas de clases con asistencia y cancelación con política", "Rutinas para tus socios (Coaching Pro)", "Setup en 1 día, sin permanencia, 7 días gratis" },
                BrandGuidelines = "Idioma es-AR (voseo). Ejemplos de copy: 'Dejá de perseguir morosos. GymHero cobra por vos.' / 'Todo lo que tu gym necesita. Nada que no.' / 'Tu gym merece dejar de perder plata.' Mostrar métricas reales, no fluff. HECHOS DEL PRODUCTO (del código, no inventar): el débito automático de cuotas existe SOLO vía MercadoPago Preapproval (el socio adhiere la tarjeta UNA vez); el auto-cobro por Chytapay NO debita — manda un LINK de pago por WhatsApp. El dolor que resolvemos: perseguir cuotas A MANO, no saber quién entró sin pagar, la venta del mostrador sin registrar y no saber cuánto queda a fin de mes. PROHIBIDO prometer: 'pagos que rebotan', lista de espera automática de clases (no implementada), app móvil nativa (es web), huella/facial sin aclarar que requieren hardware. VOCABULARIO: 'socios' y 'cuotas' (no 'alumnos').",
            ...
            new()
            {
                ...
                ContentPillars = new() { "Reservas online 24/7 con link propio del club + QR por cancha", "Señas y cobros por MercadoPago + pago dividido entre jugadores", "Agente IA por WhatsApp que reserva y cancela solo (plan Agente IA)", "Caja y POS con productos y stock: el buffet también suma", "Multi-cancha y multi-sede, hora pico y bloqueos", "Torneos con brackets + ranking ELO y niveles de jugador", "Recordatorios de reserva por WhatsApp" },
